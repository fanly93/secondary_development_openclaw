...
- Control UI/chat: add a **Saved models** picker next to the main model dropdown so chat can switch only among models declared under Models settings (`models.providers`), backed by new gateway RPC `models.listConfigured`.
...
- Bundled plugins: stage `dist-runtime` plugin skill packs as real files instead of symlinks into `dist/extensions`, so skill discovery (`realpath` containment under `./skills`) no longer drops plugin-owned skills such as `feishu-tavily-search` from the catalog and Control UI.
